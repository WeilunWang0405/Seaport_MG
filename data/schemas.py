...
class ESS(BaseModel):
    ...
    def check_soc(self):
        if not (0 <= self.min_soc <= self.initial_soc <= self.max_soc <= 1):
            raise ValueError("SOC bounds invalid: require 0 <= min <= initial <= max <= 1")
...
class ColdIroningTask(BaseModel):
    ...
    demand_fix_kw: float = Field(validation_alias=AliasChoices("demand_fix_kw", "demand_fix"))
    battery_demand_kwh: float = Field(validation_alias=AliasChoices("battery_demand_kwh", "battery_demand"))
    max_charging_power_kw: float = Field(validation_alias=AliasChoices("max_charging_power_kw", "max_charging_power", "max_charg"))
    min_charging_power_kw: float = Field(validation_alias=AliasChoices("min_charging_power_kw", "min_charging_power"))
...
class SystemData(BaseModel):
    ...
    def validate_dims_and_refs(self):
        ...
        if self.load_kw.shape != (nb, self.T):
            raise ValueError(f"load_kw must be shape (nb,T)=({nb},{self.T}), got {self.load_kw.shape}")
        if self.pv_kw.shape != (nb, self.T):
            raise ValueError(f"pv_kw must be shape (nb,T)=({nb},{self.T}), got {self.pv_kw.shape}")
