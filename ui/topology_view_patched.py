...
class TopologyView(QGraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        # --- Developer-tunable style parameters (alerts / blinking / fit) ---
        self.V_LOW_TH = 0.95
        self.V_LOW_COLOR = QColor(200, 0, 0)

        self.LOAD_GREEN_TH = 70.0
        self.LOAD_YELLOW_TH = 90.0
        self.LOAD_RED_TH = 100.0
        self.COL_LOAD_GREEN = QColor(0, 140, 0)
        self.COL_LOAD_YELLOW = QColor(200, 160, 0)
        self.COL_LOAD_RED = QColor(200, 0, 0)

        self.FIT_MARGIN = 120.0
        self.BLINK_INTERVAL_MS = 600
        self.BLINK_DIM_ALPHA = 80
        ...
        self._blink_timer.setInterval(int(getattr(self, 'BLINK_INTERVAL_MS', 600)))  # ~0.8-1 Hz perceived blink
        self._blink_timer.timeout.connect(self._toggle_blink_phase)
        self._blink_timer.start()

        # =======================
        #  Cost overlay (left/top in view, not in scene)
        # =======================
        self._cost_t = None          # shape: (T,)
        self._cost_cum = None        # shape: (T,)
        self._cost_unit = "SGD"      # 你可改成 "$" / "USD" 等
        self._cost_margin = 12

        self._cost_label = QLabel(self)
        self._cost_label.setText("Cost: ?")
        self._cost_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._cost_label.setAttribute(Qt.WA_TransparentForMouseEvents, True)
    ...
    def clear(self):
        ...
        if hasattr(self, "_cost_label") and self._cost_label is not None:
            self._cost_label.setText("Cost: -")
        self._cost_t = None
        self._cost_cum = None
    ...
    def load_topology_from_system(self, system):
        ...
        m = float(getattr(self, 'FIT_MARGIN', 120.0))
    ...
    def set_results(self, system, result):
        ...
        # ------- compute/display cost series (best-effort) -------
        self._build_cost_series()
        self._update_cost_overlay(0)  # 初始化显示 t=0

    def _build_cost_series(self):
        """Best-effort build per-time cost series from result/system."""
        self._cost_t = None
        self._cost_cum = None
        ...
        # 1) 优先：result 里直接有每时段 cost
        for attr in (
            "cost_t", "cost_time", "time_cost",
            "total_cost_t", "obj_t", "objective_t"
        ):
            if hasattr(r, attr):
                try:
                    arr = np.asarray(getattr(r, attr), dtype=float).reshape(-1)
                    self._cost_t = arr
                    self._cost_cum = np.cumsum(arr)
                    return
                except Exception:
                    pass

        # 2) 兜底：用 “电价 * 电网购电功率 * dt” 估算（仅示例，按你工程字段名适配）
        price = None
        for attr in ("price_E", "price", "price_sgd_per_kwh", "price_usd_per_kwh"):
            if sys is not None and hasattr(sys, attr):
                price = np.asarray(getattr(sys, attr), dtype=float).reshape(-1)
                break

        p_grid = None
        for attr in ("P_grid", "P_grid_kw", "p_grid", "p_grid_kw", "P_slack"):
            if hasattr(r, attr):
                p_grid = np.asarray(getattr(r, attr), dtype=float).reshape(-1)
                break

        if price is None or p_grid is None:
            return

        # dt (hour)
        dt_hr = None
        for attr in ("dt_hr", "dt_hour"):
            if sys is not None and hasattr(sys, attr):
                try:
                    dt_hr = float(getattr(sys, attr))
                except Exception:
                    pass
        if dt_hr is None:
            # 常见：dt_min / DtMin
            for attr in ("dt_min", "DtMin", "dt_minutes"):
                if sys is not None and hasattr(sys, attr):
                    try:
                        dt_hr = float(getattr(sys, attr)) / 60.0
                        break
                    except Exception:
                        pass
        if dt_hr is None:
            dt_hr = 0.5  # 你项目看起来是 30min，实在取不到就默认 0.5h

        T = int(getattr(r, "T", min(len(price), len(p_grid))))
        price = price[:T]
        p_grid = p_grid[:T]

        # 只对购电计费：export 不计费（你若要考虑上网电价可自行改）
        p_import = np.maximum(p_grid, 0.0)
        cost_t = price * p_import * dt_hr

        self._cost_t = cost_t
        self._cost_cum = np.cumsum(cost_t)

    def _update_cost_overlay(self, t: int):
        if not hasattr(self, "_cost_label") or self._cost_label is None:
            return

        if self._cost_t is None or len(self._cost_t) == 0:
            self._cost_label.setText("Cost: ?")
            return

        t = int(np.clip(t, 0, len(self._cost_t) - 1))
        ct = float(self._cost_t[t])
        cc = float(self._cost_cum[t]) if self._cost_cum is not None else float(np.sum(self._cost_t[:t+1]))

        self._cost_label.setText(
            f"Cost @ t={t}: {ct:.2f} {self._cost_unit}\n"
            f"Cumulative: {cc:.2f} {self._cost_unit}"
        )
    ...
    def update_time(self, t: int):
        ...
        for bid in r.bus_ids:
            ...
            if bid in self.bus_info_text:
                if V < float(getattr(self, 'V_LOW_TH', 0.95)):
                    col = QColor(getattr(self, 'V_LOW_COLOR', QColor(200, 0, 0)))
                    if not self._blink_phase:
                        col.setAlpha(int(getattr(self, 'BLINK_DIM_ALPHA', 80)))
    ...
    def _loading_style(self, pct: float) -> Tuple[QColor, bool]:
        ...
        g_th = float(getattr(self, "LOAD_GREEN_TH", 70.0))
        y_th = float(getattr(self, "LOAD_YELLOW_TH", 90.0))
        r_th = float(getattr(self, "LOAD_RED_TH", 100.0))

        c_g = QColor(getattr(self, "COL_LOAD_GREEN", QColor(0, 140, 0)))
        c_y = QColor(getattr(self, "COL_LOAD_YELLOW", QColor(200, 160, 0)))
        c_r = QColor(getattr(self, "COL_LOAD_RED", QColor(200, 0, 0)))
    ...
    def _draw_hv_grid_and_transformer(self, slack_bus: int):
        ...
        tx_w = float(self.tx_box_w)
        tx_h = float(self.tx_box_h)
        tx_x0 = x_c + r + float(self.tx_box_xoff)
    ...
    def _draw_load_arrow(self, bus_id: int, anchor_frac: float = 0.5):
        ...
        br = txt.boundingRect()
        txt.setPos(tip.x() - br.width()/2.0, tip.y() - br.height()/2.0)
    ...
    def _place_overlays(self):
        """Place overlay widgets at fixed view coordinates."""
        if hasattr(self, "_cost_label") and self._cost_label is not None:
            self._cost_label.move(self._cost_margin, self._cost_margin)
