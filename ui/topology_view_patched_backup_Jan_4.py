...
class TopologyStyle:
    ...
    # 1) 视图交互（缩放/平移/闪烁）
    # =============================
    zoom_step: float = 1.15          # 鼠标滚轮每次缩放倍率
    zoom_min: int = -15              # 最小缩放层级
    zoom_max: int = 25               # 最大缩放层级
    blink_interval_ms: int = 600     # 闪烁定时器间隔（越小闪得越快）
    blink_dim_alpha: int = 80        # 闪烁“暗相位”的透明度（0~255，越小越暗）

    # fitInView 预留边距（图周围空白，避免贴边）
    fit_margin: float = 120.0

    # =============================
    # 2) 布局尺度（全局间距）
    # =============================
    x_step: float = 520.0            # 水平节点间距（越大越展开）
    y_step: float = 400.0            # 垂直层间距（越大越不挤）

    # =============================
    # 3) 母线（busbar）外观
    # =============================
    busbar_len: float = 180.0        # 母线长度
    busbar_w: float = 15.0           # 母线线宽

    # =============================
    # 4) 线路（edge）外观
    # =============================
    edge_w: float = 2.0              # 线路线宽
    track_step: float = 18.0         # 线路避让偏移（并行段错开距离）
    pflow_text_offset: float = -35.0 # 潮流文字离线路的法向偏移（负数/正数决定方向）

    # 线路文字背景框 padding（让文字更清晰）
    edge_label_pad_x: float = 6.0
    edge_label_pad_y: float = 4.0

    # 箭头（线路末端）
    edge_arrow_len: float = 12.0
    edge_arrow_w: float = 6.0

    # =============================
    # 5) Slack 出线“先下再横再下”的形状参数
    # =============================
    slack_drop: float = 55.0
    slack_port_offset: float = 16.0  # slack 多条出线在母线中心点左右错开

    # =============================
    # 6) 设备框（device box）外观/布局
    # =============================
    dev_box_w: float = 160.0         # 设备框宽
    dev_box_h: float = 80.0          # 设备框高
    dev_box_gap_x: float = 18.0      # 设备框与母线水平间距
    dev_box_voffset: float = 80.0    # 设备框离母线的垂直距离（上/下共用）
    dev_box_vgap: float = 30.0       # 同一母线多个设备框的垂直间距

    # 设备连接线/箭头
    dev_link_w: float = 2.0
    dev_arrow_len: float = 12.0
    dev_arrow_w: float = 6.0

    # =============================
    # 7) Load 箭头/文字/框
    # =============================
    load_line_w: float = 1.4
    load_arrow_len: float = 12.0
    load_arrow_w: float = 6.0
    load_text_xpad: float = 8.0      # load 文本相对箭头尖端的水平偏移
    load_box_pad_x: float = 6.0      # load 文本背景框 padding
    load_box_pad_y: float = 4.0

    # =============================
    # 8) HV + 变压器（transformer）外观
    # =============================
    hv_gap: float = 340.0            # HV线 到 slack 母线的垂直距离
    hv_text_dy: float = -40.0        # HV GRID 文字相对 HV 线的竖直偏移
    hv_line_w: float = 10.0          # HV 线宽
    hv_half_len: float = 150.0       # HV 半长度最低值（会与 max_abs_x 动态取 max）
    hv_text_size: int = 20           # “HV GRID”字号
    hv_text_xpad: float = 10.0       # HV文字相对左端的x偏移
    tr_r: float = 30.0               # 变压器圆圈半径

    # 变压器 loading 框
    tx_box_w: float = 140.0
    tx_box_h: float = 44.0
    tx_box_xoff: float = 18.0        # 变压器框相对圆圈右侧的偏移
    tx_font_size: int = 18

    # =============================
    # 9) 字体大小（全局字号）
    # =============================
    font_bus_name: int = 20
    font_bus_id: int = 20
    font_edge: int = 20
    font_device: int = 20
    font_load: int = 20
    ...
    # =============================
    # 10) 颜色与阈值（展示逻辑相关，但属于“外观规则”）
    # =============================
    hv_color: QColor = field(default_factory=lambda: QColor(Qt.lightGray))
    bus_info_bg: QColor = field(default_factory=lambda: QColor(235, 235, 235))
    bus_info_border: QColor = field(default_factory=lambda: QColor(190, 190, 190))

    # 线路 loading 颜色阈值
    load_green_th: float = 70.0
    load_yellow_th: float = 90.0
    load_red_th: float = 100.0
    col_green: QColor = field(default_factory=lambda: QColor(0, 140, 0))
    col_yellow: QColor = field(default_factory=lambda: QColor(200, 160, 0))
    col_red: QColor = field(default_factory=lambda: QColor(200, 0, 0))

    # 节点欠压阈值（低于该值变红并闪烁）
    v_low_th: float = 0.95
    # 节点欠压颜色（红色）
    v_low_color: QColor = field(default_factory=lambda: QColor(200, 0, 0))
...
class TopologyView(QGraphicsView):
    ...
    # ---------------- init ----------------
    def __init__(self, parent=None, style: TopologyStyle | None = None):
        super().__init__(parent)
        self.setRenderHint(QPainter.Antialiasing, True)

        self.style = style or TopologyStyle()
        ...
        self._zoom = 0
        self._zoom_step = self.style.zoom_step
        self._zoom_min = self.style.zoom_min
        self._zoom_max = self.style.zoom_max

        self._panning = False
        self._pan_start = None

        # ---- layout/style knobs you later use as attributes ----
        self.X_STEP = self.style.x_step
        self.Y_STEP = self.style.y_step

        self.BUSBAR_LEN = self.style.busbar_len
        self.BUSBAR_W = self.style.busbar_w

        self.EDGE_W = self.style.edge_w
        self.TRACK_STEP = self.style.track_step
        self.PFLOW_TEXT_OFFSET = self.style.pflow_text_offset

        self.SLACK_DROP = self.style.slack_drop
        self.SLACK_PORT_OFFSET = self.style.slack_port_offset

        self.HV_GAP = self.style.hv_gap
        self.HV_TEXT_DY = self.style.hv_text_dy
        self.HV_W = self.style.hv_line_w
        self.HV_HALF_LEN = self.style.hv_half_len
        self.HV_COLOR = self.style.hv_color
        self.TR_R = self.style.tr_r

        self.FONT_BUS_NAME = self.style.font_bus_name
        self.FONT_BUS_ID = self.style.font_bus_id
        self.FONT_EDGE = self.style.font_edge
        self.FONT_DEVICE = self.style.font_device
        self.FONT_LOAD = self.style.font_load

        self.BUS_INFO_BG = self.style.bus_info_bg
        self.BUS_INFO_BORDER = self.style.bus_info_border

        # ---- blinking ----
        self._blink_phase = True
        self._blink_timer = QTimer(self)
        self._blink_timer.setInterval(self.style.blink_interval_ms)
        ...
        # adjacency + routing slots
        self._adj = {}
        self._pc_slot = {}
        self._pc_slot_count = {}
        self._slack_trunk_child = None
        ...
        self.edge_loading_text = {}
        self.edge_loading_box = {}

        self.device_boxes = {}
        self.device_text = {}
        self.device_link = {}

        self.load_arrow_item = {}
        self.load_arrow_head = {}
        ...
        #
        #         # ---- device box style ----
        #         self.DEV_BOX_W = self.style.dev_box_w
        #         self.DEV_BOX_H = self.style.dev_box_h
        #         self.DEV_BOX_GAP_X = self.style.dev_box_gap_x
        #         self.DEV_BOX_VOFFSET = self.style.dev_box_voffset
        #         self.DEV_BOX_VGAP = self.style.dev_box_vgap
        #
        #         # ---- fonts ----
        #         self.FONT_BUS_NAME = self.style.font_bus_name
        #         self.FONT_BUS_ID = self.style.font_bus_id
        ...
        #         self._blink_timer.start()
        #
        #         # 其它非外观的数据结构初始化保持不变（dict caches 等）
        #         ...
        ...
        # self.DEV_BOX_H = 80
        # self.DEV_GAP = 8
        # self.DEV_OFFSET = 200          # 设备框离母线的垂直距离（你觉得长就减小）
    ...
    def load_topology_from_system(self, system):
        ...
        m = float(self.style.fit_margin)
        self.fitInView(self.scene.itemsBoundingRect().adjusted(-m, -m, m, m), Qt.KeepAspectRatio)
    ...
    def update_time(self, t: int):
        ...
        for bid in r.bus_ids:
            ...
            if bid in self.bus_info_text:
                if V < self.style.v_low_th:
                    col = QColor(self.style.v_low_color)  # 拷贝一份再改 alpha
                    if not self._blink_phase:
                        col.setAlpha(self.style.blink_dim_alpha)
        ...
        for bus_id, txt in self.load_text.items():
            ...
            if bus_id in self.load_box_item:
                px = self.style.load_box_pad_x
                py = self.style.load_box_pad_y
                self.load_box_item[bus_id].setRect(
                    QRectF(txt.x() - px, txt.y() - py, br.width() + 2 * px, br.height() + 2 * py)
                )
    ...
    def _loading_style(self, pct: float) -> tuple[QColor, bool]:
        s = self.style
        if pct is None or not np.isfinite(pct):
            return QColor(0, 0, 0), False
        if pct < s.load_green_th:
            return QColor(s.col_green), False
        if pct < s.load_yellow_th:
            return QColor(s.col_yellow), False
        if pct <= s.load_red_th:
            return QColor(s.col_red), False
        return QColor(s.col_red), True
    ...
    def _draw_hv_grid_and_transformer(self, slack_bus: int):
        ...
        f = QFont()
        f.setPointSize(self.style.hv_text_size)
        f.setBold(True)
        hv_text.setFont(f)

        # always align to HV left end
        hv_text.setPos(-half_len + self.style.hv_text_xpad, hv_y + self.HV_TEXT_DY)
    ...
    def _draw_edge_routed(self, u: int, v: int):
        ...
        px = self.style.edge_label_pad_x
        py = self.style.edge_label_pad_y
        bg = InteractiveRectItem(QRectF(etxt.x() - px, etxt.y() - py, br.width() + 2 * px, br.height() + 2 * py),
    ...
    def _make_arrow_at_end(self, pts: List[QPointF]) -> QGraphicsPolygonItem:
        ...
        ux, uy = dx / L, dy / L
        arrow_len = self.style.edge_arrow_len
        arrow_w = self.style.edge_arrow_w
    ...
    def _draw_device_box(
        ...
        v0 = float(getattr(self, "DEV_BOX_VOFFSET", 80))
        vgap = float(getattr(self, "DEV_BOX_VGAP", 30))
        ...
        # ---- arrow head ----
        def add_triangle(tip: QPointF, dx: float, dy: float):
            length = self.style.dev_arrow_len
            width = self.style.dev_arrow_w
    ...
    def _draw_load_arrow(self, bus_id: int, anchor_frac: float = 0.5):
        ...
        arrow_len = self.style.load_arrow_len
        arrow_w = self.style.load_arrow_w
        ...
        br = txt.boundingRect()
        txt.setPos(tip.x() + self.style.load_text_xpad, tip.y() - br.height() / 2.0)
