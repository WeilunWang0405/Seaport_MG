...
def build_aclin_optimization(system, params: ACLinParams):
    ...
    # ---------------- Variables ----------------
    V = m.addMVar((nb, T), lb=params.v_min, ub=params.v_max, name="V")
    ...
    if nR > 0:
        ...
        # Linear thermal dynamics (explicit, consistent with your Matlab form)
        # Temp[t] = (1-a)*Temp[t-1] + a*Tout - b*p[t]
        Tout = float(params.temp_out_c)
        for r in range(nR):
            a = float(alpha[r])
            b = float(beta[r])

            # t = 0 uses initial temperature tini as "t=-1"
            m.addConstr(
                Temp[r, 0] == (1.0 - a) * float(tini[r]) + a * Tout - b * p_rf[r, 0],
                name=f"rf_temp_init[{r}]"
            )

            for t in range(1, T):
                m.addConstr(
                    Temp[r, t] == (1.0 - a) * Temp[r, t - 1] + a * Tout - b * p_rf[r, t],
                    name=f"rf_temp[{r},{t}]"
                )

            m.addConstr(Temp[r, :] >= float(tmin[r]), name=f"rf_tmin[{r}]")
            m.addConstr(Temp[r, :] <= float(tmax[r]), name=f"rf_tmax[{r}]")
    ...
    for tk in system.cold_ironing:
        bi = idx.bus_to_idx[int(tk.node_number)]
        st_raw = int(tk.start_time)
        en_raw = int(tk.end_time)

        # If data looks like 1..T, treat as 1-based with end as inclusive in the original sense,
        # but Python slice end is exclusive, so use [st-1 : en]
        if st_raw >= 1 and en_raw <= T:
            st = st_raw - 1
            en = en_raw
        else:
            st, en = st_raw, en_raw
        ...
        st = max(0, min(st, T))
        en = max(st, min(en, T))
        cold_fix_bus[bi, st:en] += float(tk.demand_fix_kw)
    ...
    # ---------------- Line active flow for limits + GUI display ----------------
    ...
    obj = gp.quicksum(
        price[t] * p_grid_pos[t] * dt_hours - params.export_price * p_grid_neg[t] * dt_hours
        for t in range(T)
    )
